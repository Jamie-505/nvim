return {
  ...
  },
  {
    ...
    build = ':TSUpdate',
    config = function()
      -- NOTE: `tmux` is deliberately absent. It was dropped from the nvim-treesitter
      -- registry, so listing it makes every `update()` log
      -- `error: Parser not available for language "tmux"`.
